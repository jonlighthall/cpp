...
struct ColumnGroup {
    ...
    size_t end_line;        // Last line where this format appears (0 = continues to end)
...
struct ColumnStructure {
    ...
    bool is_monotonic_first_column;     // True if first column increases monotonically
...
private:
    ...
    std::string generate_structure_summary(const ColumnStructure& structure) const;
};
