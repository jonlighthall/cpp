...
bool DifferenceAnalyzer::process_difference(const ColumnValues& column_data,
                                           size_t column_index,
                                           double threshold,
                                           CountStats& counter,
                                           DiffStats& differ,
    ...
    if ((diff_rounded > thresh.critical) &&
        ...
        print_hard_threshold_error(rounded1, rounded2, diff_rounded, column_index, counter);
        return false;
    }
    return true;
}
...
void DifferenceAnalyzer::process_rounded_values(const ColumnValues& column_data,
                                               size_t column_index,
                                               double rounded_diff,
                                               int minimum_deci,
                                               CountStats& counter,
    ...
    if (double big_zero = std::pow(10, -minimum_deci) / 2; rounded_diff > big_zero) {
...
void DifferenceAnalyzer::print_hard_threshold_error(double rounded1,
                                                   double rounded2,
                                                   double diff_rounded,
                                                   size_t column_index,
